...
function IosInstructions() {
    const [step, setStep] = useState(0); // 0: tap share, 1: share sheet, 2: add modal, 3: home screen
    const TOTAL = 4;
...
// ──────────────────────────────────────────────────────────────────────
// iOS phone mockup that animates through the Share → Add to Home Screen flow
// ──────────────────────────────────────────────────────────────────────
...
// Screen 0 — Safari showing MBalit, with a tapping finger on the Share button
...
function SafariNavBtn({ children, dim }: { children: React.ReactNode; dim?: boolean }) {
    return (
        <span className={`text-base ${dim ? 'text-gray-300' : 'text-[#007AFF]'}`}>{children}</span>
    );
}
...
function ShareGlyph({ highlighted }: { highlighted?: boolean }) {
    return (
        ...
        >
            {/* iOS share icon: rounded square with up-arrow exiting */}
...
// Screen 1 — Share sheet slides up, "Add to Home Screen" highlighted
function ShareSheetScreen({ tapAddToHome }: { tapAddToHome?: boolean }) {
    return (
        ...
        >
            ...
            {tapAddToHome && (
                <TapPointer style={{ left: 130, bottom: 88 }} />
            )}
        </motion.div>
    );
}
...
// Screen 2 — "Add to Home Screen" confirmation modal
                ...
                <div className="flex items-center justify-between px-3 py-2 border-b border-gray-200">
                    ...
                    <motion.span
                        ...
                        className={`text-[11px] font-bold ${
                            tapAdd ? 'text-[#0E7A3B]' : 'text-[#007AFF]'
                        }`}
                    >
                        Add
                    </motion.span>
...
// Screen 3 — Home screen with the new MBalit icon zooming in
function HomeScreen() {
    return (
        <motion.div
            initial={{ opacity: 0 }}
            ...
            className="absolute inset-0 pt-7"
            style={{
                background: 'linear-gradient(180deg, #BFE3CF 0%, #ECFDF3 60%, #FFFFFF 100%)',
            }}
        >
            <div className="grid grid-cols-4 gap-2 px-3 pt-2">
                {/* Row 1 — placeholder apps */}
...
// Animated finger-tap pointer placed by inline style
