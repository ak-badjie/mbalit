...
- **Submission + authority notifications**: hazard reports are NOT written from the client. The reporter page posts the payload to `POST /api/reports/create` with their Firebase ID token. That server-trusted route validates the request, writes the `environmentalReports` doc, then in the same call expands every `organizations` document where `isAuthority == true` to its `members` (plus `ownerId`) and creates one in-app notification per recipient in the existing `notifications` collection. Notification doc IDs are deterministic (`notif_{reportId}_{userId}`) and written with `doc.create`, so retries never overwrite read state. Notifications carry `data.deepLink: '/organization/reports'` and the notification dropdown navigates there on click. Because notifications are only ever fired here (on initial creation), they naturally stop once a report's status moves to `in_progress` or `resolved`. Requires `FIREBASE_SERVICE_ACCOUNT` (same secret used for PIN reset).
