...
- **Authority notifications**: when a hazard report is submitted, the client fires `POST /api/reports/notify` (fire-and-forget) which uses the Firebase Admin SDK to find every user with `isAuthority == true` and write one in-app notification per authority into the existing `notifications` collection. The notification payload carries `data.deepLink: '/organization/reports'` and the notification dropdown navigates there on click. Notifications are only fired on initial creation, so they naturally stop once a report is later marked `resolved`. Requires `FIREBASE_SERVICE_ACCOUNT` (same secret used for PIN reset) — without it the report is still saved, only the notification fan-out is skipped.
