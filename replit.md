...
- **Submission + authority notifications (atomic)**: hazard reports are NOT written from the client. The reporter page posts the payload — including a client-generated `requestId` (UUID) — to `POST /api/reports/create` with the user's Firebase ID token. The server-trusted route:
  1. Verifies the ID token; derives a deterministic report doc ID from `sha256(uid + ":" + requestId)`. If a report at that ID already exists, returns success without writing — making retries safe (no duplicate reports, no duplicate alerts).
  2. Resolves recipients by reading every `organizations` doc where `isAuthority == true` and unioning their `members` arrays plus `ownerId`.
  3. Stages the report doc AND every recipient's notification doc (deterministic IDs `notif_{reportId}_{userId}`) into a **single Firestore WriteBatch** and commits — so the report and notifications are persisted all-or-nothing. If the batch fails the client gets a non-2xx and can retry under the same `requestId`. Hard cap: 499 recipients per report (Firestore's 500-op batch limit minus the report doc); above this the request is rejected with 503.
- Notifications carry `data.deepLink: '/organization/reports'` and the notification dropdown navigates there on click. Because notifications are only ever written here (on initial creation), they naturally stop once a report's status moves to `in_progress` or `resolved`. Requires `FIREBASE_SERVICE_ACCOUNT` (same secret used for PIN reset).
